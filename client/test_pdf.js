...
function generateReportForTest(application) {
  ...
  const businessName = 'High Risk Test Merchant';
  const applicationId = 'APP-TEST-HIGH-RISK';
  const gstin = '27AAACH1234F1Z5';

  const riskScore = 0.783;
  ...
  const gstStatus = { status: 'Missing', text: '[○] Missing (Not Uploaded)', color: [220, 38, 38] };
  const panStatus = { status: 'Missing', text: '[○] Missing (Not Uploaded)', color: [220, 38, 38] };
  const stmtStatus = { status: 'Missing', text: '[○] Missing (Not Uploaded)', color: [220, 38, 38] };

  const totalRequiredDocs = 4;
  const missingTotalList = ['GST Certificate', 'PAN Card', 'Bank Statement'];

  const recBadgeText = 'RECOMMEND: MANUAL REVIEW';
  const recColor = [217, 119, 6];
  const recBg = [255, 251, 235];
  ...
  const outPath = path.join(outDir, 'report-High_Risk_Test_Merchant.pdf');
  fs.writeFileSync(outPath, Buffer.from(doc.output('arraybuffer')));
  console.log('[SUCCESS] Generated 2-page PDF report at:', outPath);
  console.log('Total Pages:', totalPages);
  console.log('Page 1 Status: Executive Underwriter Synthesis Badge (RECOMMEND: MANUAL REVIEW) + Tables Verified');
  console.log('Page 2 Status: Adversarial Stress Test + Multi-Agent Reasoning (6 Agents) Wrapped & Left-Aligned');
}
